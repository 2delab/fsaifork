...
class CMNodeHelloCM : public CarMakerROSInterface {
 ...
 private:
  ...
  /**
   * @brief odomFillMsg prepares the Odometry message to be transmitted from 
   * CarMaker over ROS based on Vehicle global state.
   * @param msg the actual message being prepared by this function
   */
  void odomFillMsg(nav_msgs::msg::Odometry& msg);

  /**
   * @brief imuFillMsg prepares the IMU message to be transmitted from 
   * CarMaker over ROS based on Vehicle global state.
   * @param msg the actual message being prepared by this function
   */
  void imuFillMsg(sensor_msgs::msg::Imu& msg);

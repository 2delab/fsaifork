...
void CMNodeHelloCM::odomFillMsg(nav_msgs::msg::Odometry& msg) {
  ...
  /* Position */
  msg.pose.pose.position.x = Vehicle.PoI_Pos[0];
  msg.pose.pose.position.y = Vehicle.PoI_Pos[1];
  msg.pose.pose.position.z = Vehicle.PoI_Pos[2];

  /* Orientation (convert Euler RPY to Quaternion) */
  tf2::Quaternion q;
  q.setRPY(Vehicle.Roll, Vehicle.Pitch, Vehicle.Yaw);
  msg.pose.pose.orientation = tf2::toMsg(q);

  /* Linear Velocity (in vehicle frame) */
  msg.twist.twist.linear.x = Vehicle.PoI_Vel_1[0];
  msg.twist.twist.linear.y = Vehicle.PoI_Vel_1[1];
  msg.twist.twist.linear.z = Vehicle.PoI_Vel_1[2];

  /* Angular Velocity (in vehicle frame) */
  msg.twist.twist.angular.x = Vehicle.RollVel;
  msg.twist.twist.angular.y = Vehicle.PitchVel;
  msg.twist.twist.angular.z = Vehicle.YawRate;
}
...
void CMNodeHelloCM::imuFillMsg(sensor_msgs::msg::Imu& msg) {
  ...
  /* Angular Velocity */
  msg.angular_velocity.x = Vehicle.RollVel;
  msg.angular_velocity.y = Vehicle.PitchVel;
  msg.angular_velocity.z = Vehicle.YawRate;

  /* Linear Acceleration (in vehicle frame) */
  msg.linear_acceleration.x = Vehicle.PoI_Acc_1[0];
  msg.linear_acceleration.y = Vehicle.PoI_Acc_1[1];
  msg.linear_acceleration.z = Vehicle.PoI_Acc_1[2];
}
...
int CMNodeHelloCM::userTestrunStartAtBegin() {
  ...
  { /* set up odom publisher + job */
    ...
    auto job = std::make_shared<odom_t>(nhp_, "odom");
    job->setCycleTime(10); /* 100 Hz */
  ...
  { /* set up imu publisher + job */
    ...
    auto job = std::make_shared<imu_t>(nhp_, "imu/data");
    job->setCycleTime(10); /* 100 Hz */
...
int CMNodeHelloCM::userTestrunEnd() {
  ...
  scheduler_.deleteJob("odom");
  scheduler_.deleteJob("imu/data");
  return 1;
}
